...
def apply_mutation(file_path: str, entity_name: str, new_code: str, root_dir: str | None = None) -> Tuple[bool, str]:
    """
    SOTA: CRISPR Mutations (Surgical Program Repair)
    ...
    if not entity_name.strip():
        return False, "Mutation rejected: empty entity name."
    ...
    try:
        ...
        return False, f"Mutation rejected: failed to parse source ({exc})."

    candidate_code = _extract_code_block(new_code)
    try:
        candidate_tree = ast.parse(candidate_code)
    except Exception as exc:
        return False, f"Mutation rejected: invalid proposal syntax ({exc})."

    if not candidate_tree.body:
        return False, "Mutation rejected: empty proposal."

    candidate_node = candidate_tree.body[0]
    if not isinstance(candidate_node, (ast.FunctionDef, ast.ClassDef)):
        return False, "Mutation rejected: proposal must be a function or class definition."
    if candidate_node.name != entity_name:
        return False, "Mutation rejected: proposal name does not match target entity."

    # Create a Transformer
    class CRISPRTransformer(ast.NodeTransformer):
        def __init__(self):
            self.replaced = False
    ...
    if not transformer.replaced:
        return False, "Mutation rejected: target entity not found in file."
    ...
    try:
        ...
        return False, f"Mutation rejected: invalid transformed code ({exc})."
    ...
    if test_cmd:
        ...
        if result.returncode != 0:
            ...
            return False, "Mutation rejected: validation command failed."
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_file):
        print(f"🧬 Testing CRISPR on {test_file}...")
        test_code = "def biology():\n    print('Optimized Life')"
        success, message = apply_mutation(test_file, "biology", test_code)
